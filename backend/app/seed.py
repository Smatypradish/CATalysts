...
Sources of truth:
  ...
  data/task_records_100.csv    -> the same 5 Dataset-2 rows PLUS 95 labelled
                                  synthetic rows, pre-generated by
...
def reset_and_seed():
    ...
    # ---- Dataset 2: load verbatim (source='dataset') ----
    ...
    # ---- Synthetic task-history rows (source='synthetic') ----
    # Pre-generated into data/task_records_100.csv by
    # backend/generate_task_records_100.py (documented, deterministic formula).
    # That file also embeds the 5 original rows; verify they are still identical
    # to the supplied CSV so the originals remain provably intact on every seed.
    adf = pd.read_csv(DATA_DIR / "task_records_100.csv")
    embedded = (adf[adf["Source"] == "dataset"]
                .drop(columns=["Source"]).reset_index(drop=True))
    pd.testing.assert_frame_equal(
        embedded, hdf.reset_index(drop=True), check_dtype=False,
        obj="task_records_100.csv embedded originals vs task_records.csv")
    for _, r in adf[adf["Source"] == "synthetic"].iterrows():
        db.add(TaskHistory(
            task_id=r["Task ID"], task_type=r["Task Type"], weather=r["Weather"],
            operator_skill=r["Operator Skill"], machine_age=float(r["Machine Age (yrs)"]),
            estimated_time=float(r["Estimated Time (min)"]),
            actual_time=float(r["Actual Time (min)"]), source="synthetic"))
    db.commit()
